  ...
  data() {
    ...
    return {
      ...
      DefaultConfig: {
        ...
        // CONFIG: Font
        fontsize_formats: "10px 11px 12px 14px 16px 18px 20px 24px 30px",
      ...
      this.Editor = window.tinymce.init({
        ...
        images_upload_handler: function(blobInfo, success, failure) {
          ...
          if (self.accept.indexOf(blobInfo.blob().type) > -1) {
            ...
            Http.post(`uploadImg?type=article`, formData, config)
              .then(res => {
                success(API.imgSrc + res.data.name);
                self.$emit("on-upload-complete", [res.data, success, failure]);
              })
              .catch(err => {
                failure(err);
                self.$emit("on-upload-fail", [err, success, failure]);
              });
          } else {
            failure("图片格式错误");
          }
